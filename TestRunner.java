...
public class TestRunner {
  ...
  public static void run(Class<?> testClass) {
    ...
    for (Method testMethod : testClass.getDeclaredMethods()) {
      ...
      if (count) {
        for (Method testMethodBeforeEach : testClass.getDeclaredMethods()){
          if (testMethodBeforeEach.getAnnotation(BeforeAll.class) != null){
              try {
                  testMethodBeforeEach.invoke(testObj);
    ...
    for (Method testMethod : testClass.getDeclaredMethods()) {
      ...
      if (testMethod.getAnnotation(Test.class) != null) {
        ...
        for (Method testMethodBeforeEach : testClass.getDeclaredMethods()){
          if (testMethodBeforeEach.getAnnotation(BeforeEach.class) != null){
            try {
              testMethodBeforeEach.invoke(testObj);
            } catch (IllegalAccessException | InvocationTargetException e) {
                  throw new RuntimeException(e);
        ...
        for (Method testMethodBeforeEach : testClass.getDeclaredMethods()){
          if (testMethodBeforeEach.getAnnotation(AfterEach.class) != null){
            try {
              testMethodBeforeEach.invoke(testObj);
            } catch (IllegalAccessException | InvocationTargetException e) {
              throw new RuntimeException(e);
    ...
    if (count) {
      for (Method testMethodBeforeEach : testClass.getDeclaredMethods()){
        if (testMethodBeforeEach.getAnnotation(AfterAll.class) != null){
          try {
            testMethodBeforeEach.invoke(testObj);
